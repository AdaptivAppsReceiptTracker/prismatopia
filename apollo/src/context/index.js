...
exports.context = async ({ req, connection }) => {
  ...
  if (
    ...
    logger.error("Authorization header missing from request: %O", req.headers);
    throw new AuthenticationError("Not authorized");
  }

  // Strip off the 'Bearer ' part from the header
  const encodedJWT = authorizationHeader;
...
const validateJWT = async (encodedJWT) => {
  ...
  let jwtHeader;
  try {
    const decodedToken = jwt.decode(encodedJWT, { complete: true });
    jwtHeader = /** @type {{[key: string]: any;}} */ (decodedToken).header;
  } catch (err) {
    logger.error("Error while decoding token: %O", encodedJWT, err);
    throw new AuthenticationError("Not authorized");
  }
